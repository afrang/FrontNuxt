                            ...
                            that.$axios.get(this.$url+'product/'+data[items].product)
                                .then(function (res) {
                                    ...
                                    if(data[items].option.item!=null){
                                        let myattr=res.data.data.attr.find(obj => obj.id == data[items].selectedOptions.attrvalue.id);
                                        that.mytable[items].attrname=myattr.name;
                                        let option=myattr.option.find(obj=>obj.id==data[items].selectedOptions.optionvalue.id);

                                        that.mytable[items].attroption=option.name;
                                        if (option.price!=0){

                                            that.mytable[items].price=option.price;
                                        };
                                    }else{
                                    ...
                                    if(data[items].selectedOptions.attrvalue!=null){
                                        let other=res.data.data.attr.find(obj => obj.id == data[items].option.attrvalue.id);
                                        that.mytable[items].other=other.name;
                                        let option=other.option.find(obj=>obj.id==data[items].option.optionvalue.id);
